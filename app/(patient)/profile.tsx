...
export default function PatientProfile() {
  ...
  const { data: reservations = [], refetch: refetchReservations } = useQuery({
    ...
    queryFn: () => getReservationsForPatient(user!.uid),
    enabled: !!user?.uid,
  });

  const { data: diagnosesData, refetch: refetchDiagnoses } = useQuery({
    queryKey: ["diagnoses-preview", user?.uid],
    queryFn: () => getDiagnosesForPatient(1, 3), // Fetch latest 3
    enabled: !!user?.uid,
  });
  ...
  const onRefresh = React.useCallback(async () => {
    ...
    setRefreshing(false);
  }, [refetchReservations, refetchDiagnoses]);
  ...
  const hasMore = (diagnosesData?.total || 0) > diagnoses.length || timelineEvents.length > visibleEvents.length;
    ...
    <View style={s.container}>
        ...
        >
          ...
          <View style={s.hero}>
            <View style={s.patientIdBadge}>
              <Text style={s.patientIdText}>
                PATIENT ID: {profile.id.toUpperCase()}
              </Text>
            </View>
            ...
            <View style={s.heroActions}>
              ...
              <TouchableOpacity
                style={s.newConsultBtn}
                onPress={() => router.push("/(patient)/explore")}
              >
                <Plus size={16} color={COLORS.onPrimary} />
                <Text style={s.newConsultBtnText}>New Consultation</Text>
              </TouchableOpacity>
            </View>
          </View>
          ...
          <View style={s.section}>
            <GlassCard style={s.bentoCard}>
              ...
              <TouchableOpacity
                style={s.viewAllBtn}
                onPress={() => router.push("/(patient)/diagnoses")}
              >
          ...
          <View style={[s.section, s.rowSection]}>
            <GlassCard style={[s.bentoCard, { flex: 1 }]}>
              ...
              <TouchableOpacity
                style={s.viewAllBtn}
                onPress={() => router.push("/(patient)/lab-analysis")}
              >
            ...
            <GlassCard style={[s.bentoCard, { flex: 1 }]}>
                  ...
                  diagnoses
                    ...
                    .map((v: any, i: number) => (
                      <View key={i} style={s.vaccineItem}>
                        ...
                        />
                        <View style={{ flex: 1 }}>
                          <Text style={s.vaccineName} numberOfLines={1}>{v.name}</Text>
              ...
              <TouchableOpacity
                style={s.viewAllBtn}
                onPress={() => router.push("/(patient)/vaccines")}
              >
                <Text style={s.viewAllText}>Full Log</Text>
              </TouchableOpacity>
      ...
      >
        <View style={s.modalOverlay}>
          <GlassCard style={s.logoutModal}>
            <View style={s.modalIconWrap}>
              <LogOut size={32} color={COLORS.error} />
            </View>
            <Text style={s.modalTitle}>Sign Out</Text>
            <Text style={s.modalSub}>
              Are you sure you want to end your session? You will need to log in
              again to access your records.
            </Text>
